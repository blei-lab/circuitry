...
def iter_sample_fast(iterable, samplesize, seed):
    ...
    try:
        ...
        raise ValueError("Sample larger than population.") from None
...
def gen_prompt_uniform(
    ...
    while nb_gen < N:
        ...
        name_2 = ""
        name_3 = ""
        while len({name_1, name_2, name_3}) < 3:
...
def get_name_idxs(prompts, tokenizer, idx_types=None, prepend_bos=False):
    if idx_types is None:
        idx_types = ["IO", "S", "S2"]
    name_idx_dict = {idx_type: [] for idx_type in idx_types}
    double_s2 = False
    for prompt in prompts:
        t = prompt["text"].split(" ")
        ...
        if "S" in idx_types and "S2" in idx_types:
            if name_idx_dict["S"][-1] == name_idx_dict["S2"][-1]:
                double_s2 = True
    if double_s2:
        warnings.warn("S2 index has been computed as the same for S and S2", stacklevel=1)
...
def get_word_idxs(prompts, word_list, tokenizer):
    ...
    for _pr_idx, prompt in enumerate(prompts):
        toks = [
            tokenizer.decode(t)
        ...
        for i, w_tok in enumerate(tokenized_words):
            if word_list[i] in prompt["text"]:
                ...
                except:  # noqa: E722 (allow bare except for now)
                    idx = toks.index(w_tok)
                    raise ValueError(toks, w_tok, prompt["text"])  # noqa: B904
        if idx is None:
...
def get_end_idxs(prompts, tokenizer, name_tok_len=1, prepend_bos=False, toks=None):
    ...
    for i in range(toks.shape[0]):
        ...
        try:
            ...
            print(relevant_idx)
            print(i)
            raise ValueError("Something went wrong")  # noqa: B904
        nonzers = nonzers[0]
    ...
    for i in range(toks.shape[0]):
        ...
        assert toks[i][end_idxs[i] + 1] != 0, assert_values
        assert (
...
PREFIXES = [  # noqa: F811 redefinition
    "             Afterwards,",
